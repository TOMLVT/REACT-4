...
const Tab = () => {
    ...
    const tabs = ['All' , 'Yoga' , 'Fitness' , 'Mucsles']
        ...
        <>
            <div className="container my-12 md:my-16">
                ...
                <div className="flex space-x-4 mb-4 p-6">
                    {tabs.map((tab) => (
                        <button key={tab} onClick={() => setActiveTab(tab)}   className={`py-2 px-4 rounded ${activeTab === tab ? 'bg-primary text-white' : 'bg-gray-200 text-gray-700'}`}>{tab}</button>
                    ))}
                </div>
                    {/* tab card section  */}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredCards.map((card) => (
                            ...
                            className="p-4 border rounded shadow-sm space-y-2">
                                ...
                                <p className="text-xl font-semibold mt-3">{card.title}</p>
